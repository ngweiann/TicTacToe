...
function clearBoard() {
	...
	$('.tile').html('');
	$('.tile').addClass('free');
	$('#status').html("<span style='color:black;'>Tic Tic Toe</span>");
}
...
function winAlert(player) {
	...
	if (player == "X") {
		$('#status').html("<span style='color:blue;'>Congratulations, you beat the computer!</span>")
	} else {
		$('#status').html("<span style='color:red;'>You lost!</span>")
	}
	//clearBoard();
}
...
function checkDraw() {
	...
	if ( !($('.tile').hasClass('free')) ) {
		$('#status').html("<span style='color:darkgreen;'>Draw! Try playing again!</span>");
		//clearBoard();
	}
}
...
$('.tile').on('click', function Xplay() {
	...
	if (playValid) {
		...
		$('#status').html("<span style='color:darkgreen;'>That square has already been played. Please choose another square</span>");
	}
